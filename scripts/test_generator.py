...
# @Last Modified time: 2023-08-03 17:28:54
...
tramp = 1.5e-3  # nominal pulse ramp up time (s)
    ...
    if mod_T is None:
        ...
        if trigger_source == 'INT':
            wg.set_looping_sine_burst(
                ich_sine, Fdrive, Vpp=Vpp, ncycles=.01 * DC / PRF * Fdrive, PRF=PRF, 
                tramp=tramp, ich_trig=None if ich_mod == ich_sine else ich_mod)
